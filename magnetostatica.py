...
def corrente():    #MODELO MODIFICÁVEL MODIFICAR O AUX CONFORME NECESSARIO
    ...
    '''
    #********SEM INDICAR VETOR. INDIQUE APENAS A VARIAÇÃO DE UMA COORDENADA
    '''
    j=-(S.R**2*spy.sin(S.THETA))   #INSERÇÃO DA VARIAÇÃO J 
...
vetl=[-1,1,0] #vetor linha auxiliar
ponto=[0,0,0]   #coordenada ponto


vlinha=vetl[0]*N.i+vetl[1]*N.j+vetl[2]*N.k

al=vlinha/np.sqrt(vetl[0]**2+vetl[1]**2+vetl[2]**2)

display(al)
#h=((I*(np.cos(a2)-np.cos(a1))/(4*np.pi*p))^aphi
...
def fluxo():
    ...
    #********SEM INDICAR VETOR. INDIQUE APENAS A VARIAÇÃO DE UMA COORDENADA
    '''
    b=-6*N.x*N.z+4*N.z**2*N.y+2*N.x*N.z**2   #INSERÇÃO DA VARIAÇÃO B
